...
pub async fn borrowing(bot: MyBot, msg: Message) -> ResponseResult<()> {
    bot.send_message(msg.chat.id, "En Rust, el Borrowing es una característica que permite prestar una referencia a una variable en lugar de tomar la propiedad de ella\\. Esto significa que puedes utilizar la variable sin tener que transferir la propiedad de la misma, lo que te permite usarla tanto dentro como fuera de la función donde la prestaste\\.\n\nEjemplo en Rust:\n\n`fn main() {\n   let mut s = String::from(\"Hola\");\n\n   cambiar(&mut s);\n\n   println!(\"{}\", s);\n}\n\nfn cambiar(algun_string : &mut String) {\n   algun_string.push_str(\", Mundo\");\n}`\n\nEn este ejemplo, estamos prestando una referencia mutable a la variable `s` al llamar a la función `cambiar()`\\. Dentro de la función, podemos modificar la variable a través de la referencia prestada, pero no podemos tomar propiedad de ella\\.\n\nDespués de que la función termina de ejecutarse, la propiedad de la variable vuelve a ser de la función `main()` y podemos seguir usando la variable como si no hubiéramos prestado nada\\.\n\nEs importante tener en cuenta que solo se puede prestar una referencia a una variable si ésta no esta siendo modificada en otro lugar\\. Esto se debe a que Rust utiliza el Borrowing para garantizar la seguridad de la memoria y evitar conflictos de acceso\\. Si intentáramos prestar una referencia a una variable que esta siendo modificada en otro lugar, obtendríamos un error en tiempo de compilación\\.").await?;
    bot.delete_message(msg.chat.id, msg.id).await?;
