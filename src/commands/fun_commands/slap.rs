...
pub async fn send_slap(bot: Bot, msg: Message) -> ResponseResult<()> {
    ...
    let url = nekosbest::get(nekosbest::Category::Slap).await.unwrap().url;
    bot.send_animation(msg.chat.id, InputFile::url(url.parse().unwrap()))
        .caption(format!("@{} Cacheteó a{}", username_author, username_target))
