...
/// # Errors
/// # Panics
pub async fn send_cuddle(bot: Bot, msg: Message) -> ResponseResult<()> {
    ...
    let url = nekosbest::get(nekosbest::Category::Cuddle).await.unwrap().url;
    bot.send_animation(msg.chat.id, InputFile::url(url.parse().unwrap()))
        .caption(format!("@{username_author} Está abrazando a{username_target}"))
