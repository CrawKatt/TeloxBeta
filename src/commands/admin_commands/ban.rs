...
pub async fn ban_user(bot: Bot, msg: Message) -> ResponseResult<()> {
    ...
    match msg.reply_to_message() {
        ...
        Some(replied) => {
            ...
            // Get the chat ID and user ID, and check if the person using the command is an
            // admin or owner.
            if let Some(from) = msg.from() {
                ...
                // If the user is an admin or owner, ban the user and send a message to the
                // chat. Also send a random GIF or MP4 file from the
                // "./assets/ban/" folder.
                ...
                if is_admin_or_owner {
                    ...
                    // If the user is not an admin or owner, send an error message and delete this
