...
pub async fn create_buttons(bot: Bot, msg: Message) -> ResponseResult<()> {
    ...
    bot.send_message(
        ...
        "Hola, soy un Bot que administra grupos de Telegram y seré tu asistente personal en tu \
         aprendizaje de Rust, El Lenguaje de Programación\\.",
    )
...
// options
// .iter()
// .map(|&option| InlineKeyboardButton::callback(option.to_owned(),
// option.to_owned())) .collect()
// }
//
// pub fn make_main_keyboard() -> InlineKeyboardMarkup {
// let keyboard = [
...
// match option {
// "Volver" => InlineKeyboardButton::callback(option.to_owned(),
// "back_to_main_keyboard".to_owned()),
...
// let mut keyboard: Vec<Vec<InlineKeyboardButton>> = vec![];
//
// keyboard.push(vec![InlineKeyboardButton::callback("Comandos".to_owned(),
// "Comandos".to_owned())]); keyboard.push(vec![InlineKeyboardButton::callback("
// Guía".to_owned(), "Guía".to_owned())]); keyboard.push(vec!
// [InlineKeyboardButton::callback("Soporte".to_owned(),
// "Soporte".to_owned())]); keyboard.push(vec![InlineKeyboardButton::callback("
// Volver".to_owned(), "back_to_main_keyboard".to_owned())]);
//
// InlineKeyboardMarkup::new(keyboard)
// }
