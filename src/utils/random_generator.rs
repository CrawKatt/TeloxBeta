        ...
        if let Some(user_id) = user_id {
            ...
            match file_extension {
                "gif" => {
                    let ok = bot
                        .send_animation(msg.chat.id, InputFile::file(file_path))
                        .caption(format!("{username} [<code>{}</code>] ha sido silenciado", UserId(user_id)))
                        .parse_mode(ParseMode::Html)
                ...
                _ => {
                    let err =
                        bot.send_message(msg.chat.id, "❌ No se pudo enviar el archivo").await?;
                    sleep(Duration::from_secs(60)).await;
...
pub async fn send_random_meme_generator(bot: Bot, msg: Message) -> ResponseResult<()> {
    ...
    if Path::new(&file_path).is_file() {
        // If the file path points to an existing file, check if the file name ends with ".mp4".
        // Si la ruta de archivo apunta a un archivo existente, se verifica si el nombre de archivo termina con ".mp4".
        if Path::new(&file_path).extension().map_or(false, |ext| ext.eq_ignore_ascii_case("mp4")) {
            // If the file name ends with ".mp4", send the video file to the chat using the `send_video` method of the `bot` object.
            ...
            bot.send_video(msg.chat.id, InputFile::file(&file_path))
                ...
                .reply_to_message_id(msg.id)
                .await?; // The argument provided to `send_video` is an `InputFile` object that is created from the file path.
        } else {
            // If the file name does not end with ".mp4", assume it is an image file and send it to the chat using the `send_photo` method of the `bot` object.
